...
export function sameSecret(value, expected) {
  return typeof value === 'string' && typeof expected === 'string' && expected.length >= 32 &&
...
export function validMessage(message) {
  ...
  if (message.type === 'presenter:auth') return typeof message.key === 'string' && message.key.length >= 32 && message.key.length <= 256;
  ...
  return ['tap', 'presenter:start', 'presenter:reset', 'presenter:clear'].includes(message.type);
}
