...
export function sameSecretHash(value, expectedHash) {
  return typeof value === 'string' && typeof expectedHash === 'string' && /^[a-f0-9]{64}$/.test(expectedHash) &&
    timingSafeEqual(Buffer.from(hashToken(value), 'hex'), Buffer.from(expectedHash, 'hex'));
}
...
export function validMessage(message) {
  ...
  if (message.type === 'session:create') return typeof message.masterKey === 'string' && message.masterKey.length > 0 && message.masterKey.length <= 256 &&
    typeof message.sessionKey === 'string' && message.sessionKey.length > 0 && message.sessionKey.length <= 256 &&
    typeof message.lobbyName === 'string' && message.lobbyName.trim().length > 0 && message.lobbyName.length <= 80;
  if (message.type === 'admin:auth') return typeof message.key === 'string' && message.key.length > 0 && message.key.length <= 256;
  if (message.type === 'admin:list') return true;
  if (message.type === 'boost') return typeof message.boostId === 'string' && /^[0-9]+:[0-9]+$/.test(message.boostId) && message.boostId.length <= 40;
...
export function allowedUpgrade(request, origins) {
  ...
  return pathname === '/api/ws' && typeof request.headers?.origin === 'string' && origins.has(request.headers.origin);
}
