...
export function createRaceServer(getStorage = createRedisStore, requestHandler, options = {}) {
  ...
  async function initialize() {
    ...
    if (!recoveryTimer) {
      ...
      recoveryTimer = setInterval(() => {
        if (latest) for (const ws of clients) send(ws, { type: "boostStatus", boost: boostStatus(latest, ws.playerId) });
  ...
  wss.on("connection", ws => {
    ...
    function drainTapQueue() {
      ...
      drainingTaps = true;
      const drain = () => {
        if (!queuedTaps.length || ws.readyState !== WebSocket.OPEN) {
          drainingTaps = false;
          return;
        }
        ...
        const startedAt = latest?.race.startedAt;
        pending++;
        queue = queue.then(async () => {
          if (ws.readyState !== WebSocket.OPEN) return;
          const result = await storage.store.update({ type: "tapBatch", taps, startedAt, playerToken }, playerId);
          accept(result.state);
    ...
    ws.on("message", raw => {
      ...
      if (message.type === "tap") {
        ...
        if (queuedTaps.length >= 12) {
          log("tap-queue-full", { playerId, queuedTaps: queuedTaps.length });
          return;
        }
      ...
      queue = queue.then(async () => {
        ...
        const storageMessage = message.type === "join"
          ...
          : message.type === "boost" ? { type: "boost", boostId: message.boostId, playerToken } : message;
        const result = await storage.store.update(storageMessage, playerId);
        if (result.reply) {
          if (result.reply.ok) { playerId = result.reply.id; ws.playerId = playerId; playerToken = message.playerToken; }
    ...
    ws.on("close", (code, reason) => {
      ...
      // Keep lane order, progress and final results through disconnects.
      // Players are removed by an explicit leave in the lobby or presenter clear.
    });
    ws.on("error", () => { });
  });
