...
const publicPlayers = () => [...players.values()].map(({ id,name,distance,finishedAt }) => ({ id,name,distance,finishedAt }));
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if(m.type==="join"){
      ...
      const p=players.get(id)||{id,name,distance:0,finishedAt:null,lastTapAt:0}; p.name=name; players.set(id,p); ws.playerId=id; send(ws,"joinResult",{ok:true,id}); emitState();
    } else if(m.type==="tap"){
